...
class SF_Sales_ForecastViewEdit extends ViewEdit
{
 	...
 	function display()
 	{
		...
		$("#percentage_c").prop("readonly", true);

		var sat=$("#sales_target").val();
			var optw=$("#opportunities_won").val();
			sat =parseFloat(sat.replace(',',''));
			optw =parseFloat(optw.replace(',',''));
			//alert(sat+" "+optw);
			var per=Math.round((optw/sat)*100);
    $("#percentage_c").val(per);
		$("#sales_target, #opportunities_won").keyup(function(){
			var sat=$("#sales_target").val();
			var optw=$("#opportunities_won").val();
			sat =parseFloat(sat.replace(',',''));
			optw =parseFloat(optw.replace(',',''));
